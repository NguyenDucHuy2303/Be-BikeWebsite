...
export class ProductService {
  ...
  async create(dto: any, files: any) {
    ...
    return this.prisma.product.create({
      data: {
        ...
        features: features,
        sections: sections,
        techImage: files[files.length - 1]?.buffer ?? null,
        galleryImages: galleryFiles.map((f) => f.buffer.toString('base64')),
        specifications: {}, // có thể parse từ dto nếu cần
      },
    });
  }
  ...
  async update(id: string, dto: any, galleryImages: any, techImage: any) {
